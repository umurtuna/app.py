...
# 1. AYARLAR
st.set_page_config(page_title="COA Works ERP V17.2", layout="wide")
...
if not st.session_state["authenticated"]:
    ...
    s = st.text_input("Şifre:", type="password", key="v17_2_gate")
    if st.button("Giriş"):
        if s == "NMR170":
...
if menu == "🧪 Reçete Hazırla":
    ...
    # 1. ADIM: İSİM VE MALZEME EKLEME (Bu alanlar üstte kalsın)
    col_name, col_mat, col_btn = st.columns([2, 2, 1])
    
    r_adi = col_name.text_input("Ürün İsmi:", value="urun_01", key="v17_2_pname")
    
    if 'gecici_v17_2' not in st.session_state:
        st.session_state.gecici_v17_2 = pd.DataFrame(columns=["Malzeme", "Miktar (g)"])

    m_list = sorted(data["malzemeler"].keys())
    secilen_m = col_mat.selectbox("Malzeme Seç", m_list, key="v17_2_m_sel")
    
    if col_btn.button("➕ Ekle"):
        yeni = pd.DataFrame([{"Malzeme": secilen_m, "Miktar (g)": 0.0}])
        st.session_state.gecici_v17_2 = pd.concat([st.session_state.gecici_v17_2, yeni], ignore_index=True)
        st.rerun()

    # 2. ADIM: VERİ EDİTÖRÜ
    # Editördeki değişimlerin anında algılanması için on_change yerine session_state kontrolü
    edited_data = st.data_editor(
        st.session_state.gecici_v17_2,
        num_rows="dynamic",
        use_container_width=True,
        key="v17_2_editor"
    )
    
    # Rakamları değiştirdiğin anda session_state'i güncelliyoruz
    st.session_state.gecici_v17_2 = edited_data

    # 3. ADIM: HESAPLAMA VE KOPYALAMA ALANI
    # Bu kısmı bir butonun veya bir koşulun altına gizlemiyoruz, her an çalışıyor
    if not edited_data.empty:
        ...
        # 'value' kullanımı sayesinde her hücre değişiminde metin kutusu güncellenir
        st.text_area("Kopyala ve Excel'e Yapıştır:", value=tablo_text, height=200, key="v17_2_copy")

# Diğer sayfalar (Aynen korundu)
elif menu == "📦 Envanter":
